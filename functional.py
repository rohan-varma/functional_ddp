...
def make_ddp(module: nn.Module, process_group: dist.ProcessGroup, gradient_as_bucket_view: bool = False, static_graph: bool = False, find_unused_parameters: bool = False) -> None:
    ...
    # bucket_cap_mb needs to be configurable
    bucket_bytes_cap = int(25 * 1024 * 1024)
    # Computing bucket indices and sizes
    bucket_size_limits = [
    sys.maxsize] if (
        static_graph or not find_unused_parameters) else [
    ...
    def post_forward(mod, inp, output):
        if torch.is_grad_enabled():
            ...
            pass
        if (find_unused_parameters and not static_graph) or (
            static_graph and _is_first_iter):
